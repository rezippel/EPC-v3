...
\cite{Barton1985-hz}, among which is rational function decomposition.
...
of Landau and Miller \cite{Landau1985-en} and a few new ideas,
...
\cite{Barton1985-hz,Alagar1985-hf,Gutierrez1989-ou,Von_zur_Gathen1987-xe} are equivalent to the
factorization of endomorphisms of $\A^1(\Q)$.  Kozen and Landau
\cite{Kozen1989-ns} and von zur Gathen \cite{Von_zur_Gathen1990-wd,Von_zur_Gathen1990-vk} consider
...
decomposition problem solved by Dickerson \cite{Dickerson1989-ve} deals 
with endomorphisms of $\A^1(k[\vec x])$.
...
\subsection{Endomorphisms of \texorpdfstring{$\A^n(k)$}{An(k)} 
     and \texorpdfstring{$\P^n(k)$}{Pn(k)}}
...
\subsection{Normal Subfields of \texorpdfstring{$k(x)$}{k(x)}}
...
Klein \cite{Klein1875-xw} gave the first modern, geometric proof that there
...
\cite{Jordan1876-jv} gave a proof using Sylow groups.  He extended
this work to the finite subgroups of $\mbox{SL}_2(\C)/\{\pm I\}$ in
\cite{Jordan1878-ce}.  Gordan \cite{Gordan1877-pn} considered these groups
...
As pointed out by Dorey and Whaples \cite{Dorey1974-vp} this
...
occur, is due to Ritt \cite{Ritt1922-zz} for $k = \C$, Levi \cite{Levi1942-to}
and Dorey and Whaples \cite{Dorey1974-vp} for characteristic zero and $k$
algebraically closed.  Schinzel \cite{Schinzel1982-xh} has
...
discussion, see Schinzel \cite{Schinzel1982-xh} or Lausch and
N\"obauer \cite{Lausch1973-nz}.  
...
Ritt has shown \cite{Ritt1923-lk}:
...
\cite{Fried1969-bj} for polynomials.
...
\subsection{Determining \texorpdfstring{$g$}{g} 
  from \texorpdfstring{$f$}{f} and \texorpdfstring{$h$}{h}}
...
in detail by Dickerson \cite{Dickerson1989-cy,Dickerson1989-ve} as ``computing
...
\]
Using standard techniques \cite{Knuth1997-tf} we can revert $t= h(x)$ to
...
approximate \cite{McEliece1978-aw} to this power series is the only possible
...
\cite{Brent1978-zp} it might have better asymptotic
...
\cite{Gianni1988-do} we can compute a Gr\"obner basis for $I \cap k[u]$,
...
Shannon and Sweedler \cite{Shannon1988-gh} have proven that
...
\subsection{Determination of \texorpdfstring{$h$}{h}}
\label{g:Rat:Sec}
...
\subsection{Determination of \texorpdfstring{$h$}{h} from 
   \texorpdfstring{$f$}{f} and 
   \texorpdfstring{$g$}{g}}
...
\cite{Kozen1989-ns} for decomposition over arbitrary fields.  However, they
